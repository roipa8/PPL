...
export const parseClassExp = (fields: Sexp, body: Sexp[]): Result<ClassExp> =>{
    if(body.length !== 1){
        return makeFailure("Expression not of the right form");
    }
    const bindings:Sexp=body[0];
    if(!isGoodBindings(bindings)){
    ...
    const valsResult = mapResult(binding => parseL31CExp(second(binding)), bindings);
    const bindingsResult = bind(valsResult, (vals: CExp[]) => makeOk(zipWith(makeBinding, vars, vals)));
    return bind(bindingsResult,(binding: Binding[]) =>makeOk(makeClassExp(map(makeVarDecl,fields),binding)));
}
...
const unparseClassExp= (cl: ClassExp) : string =>
    `(class (${map((b: VarDecl) => b.var,cl.fields).join(" ")}) (${map((b: Binding) => `(${b.var.var} ${unparseL31(b.val)})`, cl.methods).join(" ")}))`
