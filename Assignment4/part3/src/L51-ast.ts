...
const parseGoodClassExp = (typeName: Sexp, varDecls: Sexp, bindings: Sexp): Result<ClassExp> =>
    ! isGoodBindings(bindings) ? makeFailure(`Invalid bindings: ${JSON.stringify(bindings)}`) :
    ! (isArray(varDecls) && allT(isIdentifier,varDecls)) ? makeFailure('Invalid fields for ClassExp') :
    ! isString(typeName) ? makeFailure(`Invalid var ${typeName}`) :
    safe2((bdgs: Binding[], fields: VarDecl[]) => makeOk(makeClassExp(makeTVar(typeName),fields,bdgs)))
...
export const parsedToClassExps = (p: Parsed): ClassExp[] =>{
    if(isProgram(p)){
        const classes=p.exps.filter(isClassExp)
        const defs=p.exps.filter(isDefineExp)
        const defsVals=map((def:DefineExp)=>def.val,defs)
        const classesFromDefine=defsVals.filter(isClassExp)
        return classes.concat(classesFromDefine)
    }
    else{
        if(isDefineExp(p)){
            if(isClassExp(p.val)){
