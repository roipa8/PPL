...
export const makeTEnvFromClasses = (parsed: A.Parsed): E.TEnv => {
    const classesTExp=R.map((cl:A.ClassExp)=>A.classExpToClassTExp(cl),A.parsedToClassExps(parsed))
    return E.makeExtendTEnv(R.map((clTexp:T.ClassTExp)=>clTexp.typename,classesTExp),classesTExp,E.makeEmptyTEnv())
}
...
export const typeofDefine = (exp: A.DefineExp, tenv: E.TEnv): Result<T.VoidTExp> => {
    const constraint=bind(typeofExp(exp.val,E.makeExtendTEnv([exp.var.var],[exp.var.texp],tenv)),(x)=>checkEqualType(x, exp.var.texp,exp))
...
const typeofProgramExps = (exp: A.Exp, exps: A.Exp[], tenv: E.TEnv): Result<T.TExp> =>
    ...
    A.isCExp(exp) && isEmpty(exps) ? typeofExp(exp, tenv) :
...
export const typeofSet = (exp: A.SetExp, tenv: E.TEnv): Result<T.VoidTExp> => {
    const constraint = safe2((varTE: T.TExp, valTE: T.TExp) => checkEqualType(varTE, valTE, exp))(typeofExp(exp.var,tenv), typeofExp(exp.val,tenv));
    return bind(constraint, _=>makeOk(T.makeVoidTExp()))
};
...
export const typeofClass = (exp: A.ClassExp, tenv: E.TEnv): Result<T.TExp> => {
    const fieldTypes=R.map((vd:A.VarDecl)=>vd.texp,exp.fields)
    const fieldVars=R.map((vd:A.VarDecl)=>vd.var,exp.fields)
    const extEnv=E.makeExtendTEnv(fieldVars,fieldTypes,tenv)
    const vals=R.map((b)=>b.val,exp.methods)
    const varTEs = R.map((b) => b.var.texp, exp.methods)
    const constraints = zipWithResult((varTE, val) => bind(typeofExp(val, extEnv),
                                                           (valTE: T.TExp) => checkEqualType(varTE, valTE, exp)), varTEs, vals);
    return bind(constraints, _ =>makeOk(T.makeProcTExp(fieldTypes,A.classExpToClassTExp(exp))))
};
